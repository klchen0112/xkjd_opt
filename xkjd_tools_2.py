...
if __name__ == "__main__":
    ...
    if not os.path.exists("./data/hongyan_pinyin/hongyan_pinyin_simp_merged.yaml"):
        code_text_map = {}
        with open(
            "./data/hongyan_pinyin/hongyan_pinyin_simp_wenyi.table.bin.yaml",
            "r",
            newline="",
            encoding="utf-8",
        ) as f, open(
            "./data/hongyan_pinyin/hongyan_pinyin_simp_wenyi.pinyin_correct.txt",
            "w",
            newline="",
            encoding="utf-8",
        ) as correct_file:
            ...
            for row in reader:
                text, code, weight = row
                lazy_pinyin_code = " ".join(lazy_pinyin(text))
                weight = float(weight)
                if lazy_pinyin_code != code:
                    correct_file.write("{}\t{}\t{}\n".format(text, code, weight))
                else:
                    text_code_pair = (text, code)
                    if text_code_pair not in code_text_map:
                        code_text_map[text_code_pair] = weight
                    else:
                        code_text_map[text_code_pair] = max(
                            weight, code_text_map[text_code_pair]
                        )
            correct_file.close()
            f.close()
        with open(
            "./data/hongyan_pinyin/hongyan_pinyin_simp_supplement.table.bin.yaml",
            "r",
            newline="",
            encoding="utf-8",
        ) as f, open(
            ...
            f.close()
        with open(
            "./data/hongyan_pinyin/hongyan_pinyin_simp_merged.yaml",
            "w",
            newline="",
            encoding="utf-8",
        ) as f:
            for (char_or_words, pinyin), weight in {
                k: v
                for k, v in sorted(
                    code_text_map.items(), key=lambda item: item[1], reverse=True
                )
            }.items():
                f.write("{}\t{}\t{}\n".format(char_or_words, pinyin, weight))
            f.close()
        del code_text_map
    code_text_list = []
    with open(
        "./data/hongyan_pinyin/merged.all.dict.yaml",
        "r",
        newline="",
        encoding="utf-8",
    ) as f:
        ...
        while line:
            if line == "\n" or line == "":
                break
            char_or_words, pinyin, frequency = line.strip().split("\t")
            ...
            if word_len == 1:
                ...
                if word_len == 2:
                    ...
                    if short_pinyin not in JD_TO_PY and root_node.insert(
                        short_pinyin, char_or_words, frequency, 2
                    ):
                        continue
                    short_code = (
