...
def xkjd_process(str_pre_que,str_after_que):
        ...
        if isinstance(text, str):
            ...
            for word in jieba.lcut(text):
                if word in self.words_dict:
                    result_str += self.words_dict[word]
                else:
                    result_str += word
...
if __name__ == "__main__":
    ...
    for i in range(n_xkjd_process):
        xkjd_procs.append(Process(target=xkjd_process, args=(str_que, xkjd_que)))
